...
/**
 * Front Test Controller
 *
 * @package     Ice/Base
 * @category    Controller
 */
class TestController extends Front
{

    public function indexAction()
    {
        $this->view->setViewsDir(__ROOT__ . '/app/tmp/');
        $this->view->setPartialsDir('/');
        $this->view->setLayoutsDir('/');
        $this->view->setLayout('minimal');
        $this->view->setFile('cv');
        $this->url->setBaseUri('cv');

        $this->view->setVars([
            'title' => 'Mariusz Laczak',
            'bg' => 'mdl-color--light-green'

        ]);

        $this->config->app->name = 'Mariusz Laczak';
        $this->tag->setTitle('CV');
    }
    /**
    ...
    {
        $username = $this->dispatcher->getParam('id', null, 'user');
        $user = new UserService(new Users);
        ...
        if ($create instanceof Users) {
            $activation = $user->addRole('login');
        }
