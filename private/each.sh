...
if [ $1 ]; then
    ...
    IDS=(`/usr/bin/php $PATH/index.php --module=$MODULE --handler=$TASK --action=$ACTION`)
fi
...
if [ $2 ]; then
    ...
    if [ ${#IDS[@]} -gt 0 ]; then
        ...
        for ID in "${IDS[@]}"
        do
            OUTPUT=`/usr/bin/php $PATH/index.php --module=$MODULE --handler=$TASK2 --action=$ACTION2 $ID`
